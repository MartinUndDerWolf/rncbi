...
import gov.nih.nlm.ncbi.www.soap.eutils.efetch_seq.EFetchRequest;
import gov.nih.nlm.ncbi.www.soap.eutils.efetch_seq.EFetchResult;
...
/**
 * Methods to generate and access the results of the efetch_sequence operation
 * from the ncbi webservice.
 * 
 * @author Martin Schumann
 * 
 */
public class EFetchSequence extends EFetch {
	/**
	 * The request for the {@link EFetchSequenceServiceStub}
	 */
	private EFetchRequest request;
	/**
	 * The service.
	 */
	private EFetchSequenceServiceStub seqService;
	 ...
	 * method is called.
	 */
	public EFetchSequence() {
		// init the setter/getter methods name
		EFetchSequence.GET_NAME = "get";
		EFetchSequence.SET_NAME = "set";
		// create the request
	...
	public void fireRequest() throws RemoteException, ErrorFromWebserviceExeption {
		...
		seqService = new EFetchSequenceServiceStub();
		// get the options from the service
		options = seqService._getServiceClient().getOptions();
		// set the options
		setOptions(options);
		...
		result = seqService.run_eFetch(request);
